...
public abstract class BaseSXPElementResourceTestCase {
	...
	@Test
	public void testPostSXPElementsPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			sxpElementResource.postSXPElementsPageExportBatchHttpResponse(
				null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		SXPElement[] sxpElements = getSXPElements(exportTask);

		long totalCount = sxpElements.length;
		...
		sxpElements = getSXPElements(exportTask);

		Assert.assertEquals(totalCount + 2, sxpElements.length);

		assertContains(sxpElement1, Arrays.asList(sxpElements));
		assertContains(sxpElement2, Arrays.asList(sxpElements));
	}
	...
	protected SXPElement[] getSXPElements(ExportTask exportTask)
		throws Exception {
		...
		try {
			return SXPElementSerDes.toDTOs(
