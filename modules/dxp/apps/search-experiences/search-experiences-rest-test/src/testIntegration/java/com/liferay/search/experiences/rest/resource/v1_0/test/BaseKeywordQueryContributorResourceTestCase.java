...
public abstract class BaseKeywordQueryContributorResourceTestCase {
	...
	@Test
	public void testPostKeywordQueryContributorsPageExportBatch()
		throws Exception {
		...
		KeywordQueryContributor[] keywordQueryContributors =
			getKeywordQueryContributors(exportTask);

		long totalCount = keywordQueryContributors.length;
		...
		keywordQueryContributors = getKeywordQueryContributors(exportTask);

		Assert.assertEquals(totalCount + 2, keywordQueryContributors.length);

		assertContains(
			keywordQueryContributor1, Arrays.asList(keywordQueryContributors));
		assertContains(
			keywordQueryContributor2, Arrays.asList(keywordQueryContributors));
	}
	...
	protected KeywordQueryContributor[] getKeywordQueryContributors(
		...
		try {
			return KeywordQueryContributorSerDes.toDTOs(
