...
public abstract class BaseSXPBlueprintResourceTestCase {
	...
	@Test
	public void testPostSXPBlueprintsPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			sxpBlueprintResource.postSXPBlueprintsPageExportBatchHttpResponse(
				null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		SXPBlueprint[] sxpBlueprints = getSXPBlueprints(exportTask);

		long totalCount = sxpBlueprints.length;
		...
		sxpBlueprints = getSXPBlueprints(exportTask);

		Assert.assertEquals(totalCount + 2, sxpBlueprints.length);

		assertContains(sxpBlueprint1, Arrays.asList(sxpBlueprints));
		assertContains(sxpBlueprint2, Arrays.asList(sxpBlueprints));
	}
	...
	protected SXPBlueprint[] getSXPBlueprints(ExportTask exportTask)
		throws Exception {
		...
		try {
			return SXPBlueprintSerDes.toDTOs(
