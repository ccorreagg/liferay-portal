...
public class HeadlessDiscoveryOpenAPIResourceImpl {
	...
	public Response getGlobalOpenAPI(@PathParam("type") String type)
		...
		for (Map.Entry<String, List<String>> entry : openAPIMap.entrySet()) {
			...
			for (String openAPIPath : openAPIPaths) {
				String version = _getVersion(basePath, openAPIPath);

				Object resource = _getOpenAPIResource(basePath, version);
		...
		return _openAPIResource.mergeOpenAPIs(
			...
			basePaths, "OpenAPI specification of all Liferay REST APIs",
			responses, "Global REST API - OpenAPI", type, versions);
	}
	...
	private Object _getOpenAPIResource(String basePath, String version) {
		try {
			for (ServiceReference<Application> serviceReference1 :
				...
				String jaxRsApplicationName =
					(String)serviceReference1.getProperty(
				...
				ServiceReference<?>[] serviceReferences =
					_bundleContext.getServiceReferences(
						(String)null,
						StringBundler.concat(
							"(&(openapi.resource=true)", versionFilter, "(",
							JaxrsWhiteboardConstants.JAX_RS_APPLICATION_SELECT,
							"=\\(", JaxrsWhiteboardConstants.JAX_RS_NAME, "=",
							jaxRsApplicationName, "\\)))"));

				for (ServiceReference<?> serviceReference2 :
						serviceReferences) {
