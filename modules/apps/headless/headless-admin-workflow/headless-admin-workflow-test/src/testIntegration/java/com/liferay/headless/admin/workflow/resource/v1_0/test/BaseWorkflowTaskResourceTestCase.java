...
public abstract class BaseWorkflowTaskResourceTestCase {
	...
	public void testPostWorkflowInstanceWorkflowTasksPageExportBatch()
		...
		WorkflowTask[] workflowTasks = getWorkflowTasks(exportTask);

		long totalCount = workflowTasks.length;
		...
		if (irrelevantWorkflowInstanceId != null) {
			...
			workflowTasks = getWorkflowTasks(exportTask);

			Assert.assertEquals(1, workflowTasks.length);

			assertEquals(irrelevantWorkflowTask, workflowTasks[0]);
		}
		...
		Assert.assertEquals(totalCount + 2, workflowTasks.length);

		assertContains(workflowTask1, Arrays.asList(workflowTasks));
		assertContains(workflowTask2, Arrays.asList(workflowTasks));
	}
	...
	protected WorkflowTask[] getWorkflowTasks(ExportTask exportTask)
		throws Exception {
		...
		try {
			return WorkflowTaskSerDes.toDTOs(
