...
public abstract class BaseBlogPostingResourceTestCase {
	...
	@Test
	public void testPostSiteBlogPostingsPageExportBatch() throws Exception {
		Long siteId = testGetSiteBlogPostingsPage_getSiteId();
		Long irrelevantSiteId =
			testGetSiteBlogPostingsPage_getIrrelevantSiteId();

		HttpInvoker.HttpResponse httpResponse =
			blogPostingResource.postSiteBlogPostingsPageExportBatchHttpResponse(
				siteId, null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		BlogPosting[] blogPostings = getBlogPostings(exportTask);

		long totalCount = blogPostings.length;
		...
		if (irrelevantSiteId != null) {
			...
			blogPostings = getBlogPostings(exportTask);

			Assert.assertEquals(1, blogPostings.length);

			assertEquals(irrelevantBlogPosting, blogPostings[0]);
		}
		...
		Assert.assertEquals(totalCount + 2, blogPostings.length);

		assertContains(blogPosting1, Arrays.asList(blogPostings));
		assertContains(blogPosting2, Arrays.asList(blogPostings));
	}
	...
	protected BlogPosting[] getBlogPostings(ExportTask exportTask)
		throws Exception {
		...
		try {
			return BlogPostingSerDes.toDTOs(
