...
public abstract class BaseKnowledgeBaseArticleResourceTestCase {
	...
	@Test
	public void testPostKnowledgeBaseFolderKnowledgeBaseArticlesPageExportBatch()
		throws Exception {
		...
		KnowledgeBaseArticle[] knowledgeBaseArticles = getKnowledgeBaseArticles(
			exportTask);

		long totalCount = knowledgeBaseArticles.length;
		...
		if (irrelevantKnowledgeBaseFolderId != null) {
			...
			knowledgeBaseArticles = getKnowledgeBaseArticles(exportTask);

			Assert.assertEquals(1, knowledgeBaseArticles.length);

			assertEquals(
				irrelevantKnowledgeBaseArticle, knowledgeBaseArticles[0]);
		}
		...
		Assert.assertEquals(totalCount + 2, knowledgeBaseArticles.length);

		assertContains(
			knowledgeBaseArticle1, Arrays.asList(knowledgeBaseArticles));
		assertContains(
			knowledgeBaseArticle2, Arrays.asList(knowledgeBaseArticles));
	}
	...
	@Test
	public void testPostSiteKnowledgeBaseArticlesPageExportBatch()
		throws Exception {
	...
	protected KnowledgeBaseArticle[] getKnowledgeBaseArticles(
		...
		try {
			return KnowledgeBaseArticleSerDes.toDTOs(
