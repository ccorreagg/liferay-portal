...
public abstract class BaseKnowledgeBaseFolderResourceTestCase {
	...
	@Test
	public void testPostSiteKnowledgeBaseFoldersPageExportBatch()
		throws Exception {
		...
		KnowledgeBaseFolder[] knowledgeBaseFolders = getKnowledgeBaseFolders(
			exportTask);

		long totalCount = knowledgeBaseFolders.length;
		...
		if (irrelevantSiteId != null) {
			...
			knowledgeBaseFolders = getKnowledgeBaseFolders(exportTask);

			Assert.assertEquals(1, knowledgeBaseFolders.length);

			assertEquals(
				irrelevantKnowledgeBaseFolder, knowledgeBaseFolders[0]);
		}
		...
		Assert.assertEquals(totalCount + 2, knowledgeBaseFolders.length);

		assertContains(
			knowledgeBaseFolder1, Arrays.asList(knowledgeBaseFolders));
		assertContains(
			knowledgeBaseFolder2, Arrays.asList(knowledgeBaseFolders));
	}
	...
	protected KnowledgeBaseFolder[] getKnowledgeBaseFolders(
		...
		try {
			return KnowledgeBaseFolderSerDes.toDTOs(
