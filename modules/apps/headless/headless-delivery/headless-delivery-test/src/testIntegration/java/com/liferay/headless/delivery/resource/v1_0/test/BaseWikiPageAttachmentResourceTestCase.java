...
public abstract class BaseWikiPageAttachmentResourceTestCase {
	...
	@Test
	public void testPostWikiPageWikiPageAttachmentsPageExportBatch()
		throws Exception {
		...
		HttpInvoker.HttpResponse httpResponse =
			wikiPageAttachmentResource.
				postWikiPageWikiPageAttachmentsPageExportBatchHttpResponse(
					wikiPageId, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		WikiPageAttachment[] wikiPageAttachments = getWikiPageAttachments(
			exportTask);

		long totalCount = wikiPageAttachments.length;
		...
		if (irrelevantWikiPageId != null) {
			...
			httpResponse =
				wikiPageAttachmentResource.
					postWikiPageWikiPageAttachmentsPageExportBatchHttpResponse(
						irrelevantWikiPageId, null, null, null);

			exportTask = ExportTask.toDTO(httpResponse.getContent());

			wikiPageAttachments = getWikiPageAttachments(exportTask);

			Assert.assertEquals(1, wikiPageAttachments.length);

			assertEquals(irrelevantWikiPageAttachment, wikiPageAttachments[0]);
		}
		...
		Assert.assertEquals(totalCount + 2, wikiPageAttachments.length);

		assertContains(wikiPageAttachment1, Arrays.asList(wikiPageAttachments));
		assertContains(wikiPageAttachment2, Arrays.asList(wikiPageAttachments));
	}
	...
	protected WikiPageAttachment[] getWikiPageAttachments(ExportTask exportTask)
		throws Exception {
		...
		try {
			return WikiPageAttachmentSerDes.toDTOs(
