...
public abstract class BaseFormResourceTestCase {
	...
	@Test
	public void testPostSiteFormsPageExportBatch() throws Exception {
		Long siteId = testGetSiteFormsPage_getSiteId();
		Long irrelevantSiteId = testGetSiteFormsPage_getIrrelevantSiteId();

		HttpInvoker.HttpResponse httpResponse =
			formResource.postSiteFormsPageExportBatchHttpResponse(
				siteId, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		Form[] forms = getForms(exportTask);

		long totalCount = forms.length;
		...
		if (irrelevantSiteId != null) {
			...
			forms = getForms(exportTask);

			Assert.assertEquals(1, forms.length);

			assertEquals(irrelevantForm, forms[0]);
		}
		...
		httpResponse = formResource.postSiteFormsPageExportBatchHttpResponse(
			siteId, null, null, null);
		...
		Assert.assertEquals(totalCount + 2, forms.length);

		assertContains(form1, Arrays.asList(forms));
		assertContains(form2, Arrays.asList(forms));
	}
	...
	protected Form[] getForms(ExportTask exportTask) throws Exception {
		...
		try {
			return FormSerDes.toDTOs(zipReader.getEntryAsString("export.json"));
		}
		finally {
			zipReader.close();
		}
