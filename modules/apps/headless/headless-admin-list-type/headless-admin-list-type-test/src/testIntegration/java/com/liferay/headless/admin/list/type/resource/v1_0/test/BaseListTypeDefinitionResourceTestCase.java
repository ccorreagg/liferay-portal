...
public abstract class BaseListTypeDefinitionResourceTestCase {
	...
	@Test
	public void testPostListTypeDefinitionsPageExportBatch() throws Exception {
		...
		ListTypeDefinition[] listTypeDefinitions = getListTypeDefinitions(
			exportTask);

		long totalCount = listTypeDefinitions.length;
		...
		listTypeDefinitions = getListTypeDefinitions(exportTask);

		Assert.assertEquals(totalCount + 2, listTypeDefinitions.length);

		assertContains(listTypeDefinition1, Arrays.asList(listTypeDefinitions));
		assertContains(listTypeDefinition2, Arrays.asList(listTypeDefinitions));
	}
	...
	protected ListTypeDefinition[] getListTypeDefinitions(ExportTask exportTask)
		throws Exception {
		...
		try {
			return ListTypeDefinitionSerDes.toDTOs(
