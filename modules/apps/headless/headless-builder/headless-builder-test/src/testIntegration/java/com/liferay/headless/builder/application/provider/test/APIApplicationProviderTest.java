...
public class APIApplicationProviderTest {
			...
			() -> {
				HTTPTestUtil.invoke(
					...
					"headless-builder/schemas/by-external-reference-code" +
						"/SCHEMA/requestAPISchemaToAPIEndpoints/ENDPOINT",
					Http.Method.PUT);
				HTTPTestUtil.invoke(
					null,
					"headless-builder/schemas/by-external-reference-code" +
						"/SCHEMA/responseAPISchemaToAPIEndpoints/ENDPOINT",
					Http.Method.PUT);
				...
				List<APIApplication.Endpoint> endpoints =
				...
				List<APIApplication.Property> properties =
					schema.getProperties();

				Assert.assertEquals(
					properties.toString(), 1, properties.size());
	...
	private void _withAPIApplication(
			JSONObject apiApplicationJSONObject,
			UnsafeRunnable<Exception> unsafeRunnable)
		throws Exception {

		JSONObject jsonObject = HTTPTestUtil.invoke(
			apiApplicationJSONObject.toString(),
		...
		try {
			...
			HTTPTestUtil.invoke(
				null,
				"headless-builder/applications/" + jsonObject.getString("id"),
				Http.Method.DELETE);
		}
	}
