...
public abstract class BaseUserGroupResourceTestCase {
	...
	@Test
	public void testPostUserGroupsPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			userGroupResource.postUserGroupsPageExportBatchHttpResponse(
				null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		UserGroup[] userGroups = getUserGroups(exportTask);

		long totalCount = userGroups.length;
		...
		userGroups = getUserGroups(exportTask);

		Assert.assertEquals(totalCount + 2, userGroups.length);

		assertContains(userGroup1, Arrays.asList(userGroups));
		assertContains(userGroup2, Arrays.asList(userGroups));
	}
	...
	protected UserGroup[] getUserGroups(ExportTask exportTask)
		throws Exception {
