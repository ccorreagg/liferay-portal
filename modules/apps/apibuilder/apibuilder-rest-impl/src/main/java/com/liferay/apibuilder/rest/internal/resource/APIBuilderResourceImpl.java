...
@Component(
	property = {
		"osgi.jaxrs.application.select=(osgi.jaxrs.name=Liferay.APIBuilder)",
		"osgi.jaxrs.resource=true"
	},
	scope = ServiceScope.PROTOTYPE, service = APIBuilderResource.class
)
public class APIBuilderResourceImpl implements APIBuilderResource {
	...
	public Response get() throws Exception {
		...
		Operation operation = _operationProvider.getOperation(
			_portal.getCompanyId(_httpServletRequest), Method.GET,
		...
		MediaType mediaType = MediaType.parse(
			_httpServletRequest.getHeader(HttpHeaders.ACCEPT));
	...
	private OperationContext _getOperationContext(
		...
		for (AttributeValue attributeValue :
				_getAttributeValues(
					operation, _httpServletRequest, mediaType)) {
