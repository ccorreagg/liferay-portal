...
public abstract class BaseDSEnvelopeResourceTestCase {
	...
	@Test
	public void testPostSiteDSEnvelopesPageExportBatch() throws Exception {
		Long siteId = testGetSiteDSEnvelopesPage_getSiteId();
		Long irrelevantSiteId =
			testGetSiteDSEnvelopesPage_getIrrelevantSiteId();

		HttpInvoker.HttpResponse httpResponse =
			dsEnvelopeResource.postSiteDSEnvelopesPageExportBatchHttpResponse(
				siteId, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		DSEnvelope[] dsEnvelopes = getDSEnvelopes(exportTask);

		long totalCount = dsEnvelopes.length;
		...
		if (irrelevantSiteId != null) {
			...
			dsEnvelopes = getDSEnvelopes(exportTask);

			Assert.assertEquals(1, dsEnvelopes.length);

			assertEquals(irrelevantDSEnvelope, dsEnvelopes[0]);
		}
		...
		Assert.assertEquals(totalCount + 2, dsEnvelopes.length);

		assertContains(dsEnvelope1, Arrays.asList(dsEnvelopes));
		assertContains(dsEnvelope2, Arrays.asList(dsEnvelopes));
	}
	...
	protected DSEnvelope[] getDSEnvelopes(ExportTask exportTask)
		throws Exception {
		...
		try {
			return DSEnvelopeSerDes.toDTOs(
