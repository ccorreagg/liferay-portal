...
public abstract class BaseObjectActionResourceTestCase {
	...
	@Test
	public void testPostObjectDefinitionObjectActionsPageExportBatch()
		throws Exception {
		...
		ObjectAction[] objectActions = getObjectActions(exportTask);

		long totalCount = objectActions.length;
		...
		if (irrelevantObjectDefinitionId != null) {
			...
			objectActions = getObjectActions(exportTask);

			Assert.assertEquals(1, objectActions.length);

			assertEquals(irrelevantObjectAction, objectActions[0]);
		}
		...
		Assert.assertEquals(totalCount + 2, objectActions.length);

		assertContains(objectAction1, Arrays.asList(objectActions));
		assertContains(objectAction2, Arrays.asList(objectActions));
	}
	...
	protected ObjectAction[] getObjectActions(ExportTask exportTask)
		throws Exception {
		...
		try {
			return ObjectActionSerDes.toDTOs(
