...
public abstract class BaseObjectDefinitionResourceTestCase {
	...
	@Test
	public void testPostObjectDefinitionsPageExportBatch() throws Exception {
		...
		ObjectDefinition[] objectDefinitions = getObjectDefinitions(exportTask);

		long totalCount = objectDefinitions.length;
		...
		objectDefinitions = getObjectDefinitions(exportTask);

		Assert.assertEquals(totalCount + 2, objectDefinitions.length);

		assertContains(objectDefinition1, Arrays.asList(objectDefinitions));
		assertContains(objectDefinition2, Arrays.asList(objectDefinitions));
	}
	...
	protected ObjectDefinition[] getObjectDefinitions(ExportTask exportTask)
		throws Exception {
		...
		try {
			return ObjectDefinitionSerDes.toDTOs(
