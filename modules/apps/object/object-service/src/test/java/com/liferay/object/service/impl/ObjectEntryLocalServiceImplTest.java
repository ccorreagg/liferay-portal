...
public class ObjectEntryLocalServiceImplTest {
	...
	public void setUp() throws Exception {
		...
		ReflectionTestUtil.setFieldValue(
			_objectEntryLocalServiceImpl, "_objectDefinitionPersistence",
			Mockito.mock(ObjectDefinitionPersistence.class));
		...
		Mockito.when(
			objectDefinitionPersistence.findByPrimaryKey(Mockito.anyLong())
		).thenReturn(
	...
	private void _applyOrderBy(int start, int end, Sort[] sorts)
		...
		Method method = ObjectEntryLocalServiceImpl.class.getDeclaredMethod(
			"_applyOrderBy", DSLQuery.class, long.class, int.class, int.class,
			Sort[].class);
		...
		method.invoke(
			...
			RandomTestUtil.randomLong(), start, end, sorts);
	}
