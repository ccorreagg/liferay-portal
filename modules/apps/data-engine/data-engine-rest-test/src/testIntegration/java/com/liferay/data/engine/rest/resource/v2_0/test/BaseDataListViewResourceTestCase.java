...
public abstract class BaseDataListViewResourceTestCase {
	...
	@Test
	public void testPostDataDefinitionDataListViewsPageExportBatch()
		throws Exception {
		...
		DataListView[] dataListViews = getDataListViews(exportTask);

		long totalCount = dataListViews.length;
		...
		if (irrelevantDataDefinitionId != null) {
			...
			dataListViews = getDataListViews(exportTask);

			Assert.assertEquals(1, dataListViews.length);

			assertEquals(irrelevantDataListView, dataListViews[0]);
		}
		...
		Assert.assertEquals(totalCount + 2, dataListViews.length);

		assertContains(dataListView1, Arrays.asList(dataListViews));
		assertContains(dataListView2, Arrays.asList(dataListViews));
	}
	...
	protected DataListView[] getDataListViews(ExportTask exportTask)
		throws Exception {
		...
		try {
			return DataListViewSerDes.toDTOs(
