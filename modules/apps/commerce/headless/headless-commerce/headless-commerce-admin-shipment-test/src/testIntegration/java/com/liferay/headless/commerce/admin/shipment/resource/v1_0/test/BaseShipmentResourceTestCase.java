...
public abstract class BaseShipmentResourceTestCase {
	...
	@Test
	public void testPostShipmentsPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			shipmentResource.postShipmentsPageExportBatchHttpResponse(
				null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		Shipment[] shipments = getShipments(exportTask);

		long totalCount = shipments.length;
		...
		shipments = getShipments(exportTask);

		Assert.assertEquals(totalCount + 2, shipments.length);

		assertContains(shipment1, Arrays.asList(shipments));
		assertContains(shipment2, Arrays.asList(shipments));
	}
	...
	protected Shipment[] getShipments(ExportTask exportTask) throws Exception {
