...
public abstract class BasePlacedOrderItemResourceTestCase {
	...
	public void testPostPlacedOrderPlacedOrderItemsPageExportBatch()
		...
		PlacedOrderItem[] placedOrderItems = getPlacedOrderItems(exportTask);

		long totalCount = placedOrderItems.length;
		...
		if (irrelevantPlacedOrderId != null) {
			...
			httpResponse =
				placedOrderItemResource.
					postPlacedOrderPlacedOrderItemsPageExportBatchHttpResponse(
						irrelevantPlacedOrderId, null, null, null, null);

			exportTask = ExportTask.toDTO(httpResponse.getContent());

			placedOrderItems = getPlacedOrderItems(exportTask);

			Assert.assertEquals(1, placedOrderItems.length);

			assertEquals(irrelevantPlacedOrderItem, placedOrderItems[0]);
		}
		...
		Assert.assertEquals(totalCount + 2, placedOrderItems.length);

		assertContains(placedOrderItem1, Arrays.asList(placedOrderItems));
		assertContains(placedOrderItem2, Arrays.asList(placedOrderItems));
	}
	...
	protected PlacedOrderItem[] getPlacedOrderItems(ExportTask exportTask)
		throws Exception {
		...
		try {
			return PlacedOrderItemSerDes.toDTOs(
