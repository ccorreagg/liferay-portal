...
public abstract class BasePlacedOrderItemShipmentResourceTestCase {
	...
	@Test
	public void testPostPlacedOrderItemPlacedOrderItemShipmentsPageExportBatch()
		throws Exception {
		...
		HttpInvoker.HttpResponse httpResponse =
			placedOrderItemShipmentResource.
				postPlacedOrderItemPlacedOrderItemShipmentsPageExportBatchHttpResponse(
					placedOrderItemId, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		PlacedOrderItemShipment[] placedOrderItemShipments =
			getPlacedOrderItemShipments(exportTask);

		long totalCount = placedOrderItemShipments.length;
		...
		if (irrelevantPlacedOrderItemId != null) {
			...
			httpResponse =
				placedOrderItemShipmentResource.
					postPlacedOrderItemPlacedOrderItemShipmentsPageExportBatchHttpResponse(
						irrelevantPlacedOrderItemId, null, null, null);

			exportTask = ExportTask.toDTO(httpResponse.getContent());

			placedOrderItemShipments = getPlacedOrderItemShipments(exportTask);

			Assert.assertEquals(1, placedOrderItemShipments.length);

			assertEquals(
				irrelevantPlacedOrderItemShipment, placedOrderItemShipments[0]);
		}
		...
		Assert.assertEquals(totalCount + 2, placedOrderItemShipments.length);

		assertContains(
			placedOrderItemShipment1, Arrays.asList(placedOrderItemShipments));
		assertContains(
			placedOrderItemShipment2, Arrays.asList(placedOrderItemShipments));
	}
	...
	protected PlacedOrderItemShipment[] getPlacedOrderItemShipments(
		...
		try {
			return PlacedOrderItemShipmentSerDes.toDTOs(
