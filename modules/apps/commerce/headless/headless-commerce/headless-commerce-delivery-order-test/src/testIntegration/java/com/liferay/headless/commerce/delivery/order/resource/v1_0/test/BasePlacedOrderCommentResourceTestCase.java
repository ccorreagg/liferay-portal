...
public abstract class BasePlacedOrderCommentResourceTestCase {
	...
	@Test
	public void testPostPlacedOrderPlacedOrderCommentsPageExportBatch()
		throws Exception {
		...
		long totalCount = placedOrderComments.length;
		...
		if (irrelevantPlacedOrderId != null) {
			...
			httpResponse =
				placedOrderCommentResource.
					postPlacedOrderPlacedOrderCommentsPageExportBatchHttpResponse(
						irrelevantPlacedOrderId, null, null, null);

			exportTask = ExportTask.toDTO(httpResponse.getContent());

			placedOrderComments = getPlacedOrderComments(exportTask);

			Assert.assertEquals(1, placedOrderComments.length);

			assertEquals(irrelevantPlacedOrderComment, placedOrderComments[0]);
		}
		...
		Assert.assertEquals(totalCount + 2, placedOrderComments.length);

		assertContains(placedOrderComment1, Arrays.asList(placedOrderComments));
		assertContains(placedOrderComment2, Arrays.asList(placedOrderComments));
	}
	...
	protected PlacedOrderComment[] getPlacedOrderComments(ExportTask exportTask)
		throws Exception {
		...
		try {
			return PlacedOrderCommentSerDes.toDTOs(
