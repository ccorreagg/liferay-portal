...
public abstract class BaseWarehouseResourceTestCase {
	...
	@Test
	public void testPostWarehousesPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			warehouseResource.postWarehousesPageExportBatchHttpResponse(
				null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		Warehouse[] warehouses = getWarehouses(exportTask);

		long totalCount = warehouses.length;
		...
		warehouses = getWarehouses(exportTask);

		Assert.assertEquals(totalCount + 2, warehouses.length);

		assertContains(warehouse1, Arrays.asList(warehouses));
		assertContains(warehouse2, Arrays.asList(warehouses));
	}
