...
public abstract class BaseOrderTypeResourceTestCase {
	...
	@Test
	public void testPostOrderTypesPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			orderTypeResource.postOrderTypesPageExportBatchHttpResponse(
				null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		OrderType[] orderTypes = getOrderTypes(exportTask);

		long totalCount = orderTypes.length;
		...
		orderTypes = getOrderTypes(exportTask);

		Assert.assertEquals(totalCount + 2, orderTypes.length);

		assertContains(orderType1, Arrays.asList(orderTypes));
		assertContains(orderType2, Arrays.asList(orderTypes));
	}
