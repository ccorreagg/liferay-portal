...
public abstract class BaseTermResourceTestCase {
	...
	@Test
	public void testPostTermsPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			termResource.postTermsPageExportBatchHttpResponse(
				null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		Term[] terms = getTerms(exportTask);

		long totalCount = terms.length;

		Term term1 = testGetTermsPage_addTerm(randomTerm());

		Term term2 = testGetTermsPage_addTerm(randomTerm());

		httpResponse = termResource.postTermsPageExportBatchHttpResponse(
			null, null, null, null, null, null);

		exportTask = ExportTask.toDTO(httpResponse.getContent());

		terms = getTerms(exportTask);

		Assert.assertEquals(totalCount + 2, terms.length);

		assertContains(term1, Arrays.asList(terms));
		assertContains(term2, Arrays.asList(terms));
	}
	...
	protected Term[] getTerms(ExportTask exportTask) throws Exception {
		...
		try {
			return TermSerDes.toDTOs(zipReader.getEntryAsString("export.json"));
		}
		finally {
			zipReader.close();
		}
