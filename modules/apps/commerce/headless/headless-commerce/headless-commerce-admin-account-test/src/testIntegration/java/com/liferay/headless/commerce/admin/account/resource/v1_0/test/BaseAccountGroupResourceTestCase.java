...
public abstract class BaseAccountGroupResourceTestCase {
	...
	@Test
	public void testPostAccountGroupsPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			accountGroupResource.postAccountGroupsPageExportBatchHttpResponse(
				null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		AccountGroup[] accountGroups = getAccountGroups(exportTask);

		long totalCount = accountGroups.length;
		...
		accountGroups = getAccountGroups(exportTask);

		Assert.assertEquals(totalCount + 2, accountGroups.length);

		assertContains(accountGroup1, Arrays.asList(accountGroups));
		assertContains(accountGroup2, Arrays.asList(accountGroups));
	}
