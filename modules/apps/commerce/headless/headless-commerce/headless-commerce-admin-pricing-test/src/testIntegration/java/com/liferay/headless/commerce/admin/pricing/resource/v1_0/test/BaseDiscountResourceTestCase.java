...
public abstract class BaseDiscountResourceTestCase {
	...
	@Test
	public void testPostDiscountsPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			discountResource.postDiscountsPageExportBatchHttpResponse(
				null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		Discount[] discounts = getDiscounts(exportTask);

		long totalCount = discounts.length;
		...
		discounts = getDiscounts(exportTask);

		Assert.assertEquals(totalCount + 2, discounts.length);

		assertContains(discount1, Arrays.asList(discounts));
		assertContains(discount2, Arrays.asList(discounts));
	}
	...
	protected Discount[] getDiscounts(ExportTask exportTask) throws Exception {
