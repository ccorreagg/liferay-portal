...
public abstract class BaseProductGroupResourceTestCase {
	...
	@Test
	public void testPostProductGroupsPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			productGroupResource.postProductGroupsPageExportBatchHttpResponse(
				null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		ProductGroup[] productGroups = getProductGroups(exportTask);

		long totalCount = productGroups.length;
		...
		productGroups = getProductGroups(exportTask);

		Assert.assertEquals(totalCount + 2, productGroups.length);

		assertContains(productGroup1, Arrays.asList(productGroups));
		assertContains(productGroup2, Arrays.asList(productGroups));
	}
