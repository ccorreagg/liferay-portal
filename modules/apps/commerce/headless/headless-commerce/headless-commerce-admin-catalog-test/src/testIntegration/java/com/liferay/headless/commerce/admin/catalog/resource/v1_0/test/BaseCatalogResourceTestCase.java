...
public abstract class BaseCatalogResourceTestCase {
	...
	@Test
	public void testPostCatalogsPageExportBatch() throws Exception {
		HttpInvoker.HttpResponse httpResponse =
			catalogResource.postCatalogsPageExportBatchHttpResponse(
				null, null, null, null, null, null);

		ExportTask exportTask = ExportTask.toDTO(httpResponse.getContent());

		Catalog[] catalogs = getCatalogs(exportTask);

		long totalCount = catalogs.length;

		Catalog catalog1 = testGetCatalogsPage_addCatalog(randomCatalog());

		Catalog catalog2 = testGetCatalogsPage_addCatalog(randomCatalog());

		httpResponse = catalogResource.postCatalogsPageExportBatchHttpResponse(
			null, null, null, null, null, null);

		exportTask = ExportTask.toDTO(httpResponse.getContent());

		catalogs = getCatalogs(exportTask);

		Assert.assertEquals(totalCount + 2, catalogs.length);

		assertContains(catalog1, Arrays.asList(catalogs));
		assertContains(catalog2, Arrays.asList(catalogs));
	}
	...
	protected Catalog[] getCatalogs(ExportTask exportTask) throws Exception {
